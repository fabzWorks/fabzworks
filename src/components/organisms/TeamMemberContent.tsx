...
// lucide-react no longer ships trademarked brand marks, so social links
// use generic, distinguishable icons instead.
const iconMap = {
  linkedin: Link2,
  github: Code2,
  twitter: AtSign,
  ...
  return (
    <div>
      <div className="flex flex-col items-center text-center sm:flex-row sm:items-start sm:text-left sm:gap-5">
        <div
          className={`flex h-24 w-24 shrink-0 items-center justify-center rounded-3xl bg-gradient-to-br ${member.gradient} text-2xl font-bold text-white shadow-lg`}
        >
          {member.initials}
        </div>
        <div className="mt-4 sm:mt-1">
          <h1 className="text-2xl font-bold text-[var(--text)]">{member.name}</h1>
          <p className="text-sm font-medium text-[var(--accent)]">{member.role}</p>
          <div className="mt-3 flex flex-wrap justify-center gap-3 text-xs text-[var(--text-faint)] sm:justify-start">
            <span className="flex items-center gap-1.5">
              <Briefcase size={13} /> {member.experience}
            </span>
            <span className="flex items-center gap-1.5">
